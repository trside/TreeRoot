...
#define TR_BIND_EVENT(fn) std::bind(&fn, this, std::placeholders::_1)		// std::bind()方法绑定类成员函数时需要传入该对象本身的指针作为第二个参数，以确保绑定后的成员函数归属于这个对象，而在实际的调用时，传入的第一个参数是bind方法中的第三个参数
