...
namespace tr {
	...
	Application::Application()
	{
		...
		m_MainCamera.reset(new Camera(m_Window->GetWidth(), m_Window->GetHeight()));
	}
	Application::~Application()
	{
	}
	...
	void Application::Run() 
	{
		while (m_Running)
		{
			RenderCommand::SetClearColor({ 0.25, 0.25, 0.25, 1.0 });
			RenderCommand::Clear();
			...
			if (Input::GetMouseButtonDown(TR_MOUSE_BUTTON_1))
				TR_CORE_INFO("down!!");
			if (Input::GetMouseButton(TR_MOUSE_BUTTON_1))
				TR_CORE_INFO("hold!!");
			if (Input::GetMouseButtonUp(TR_MOUSE_BUTTON_1))
				TR_CORE_INFO("up!!");
